...
在「设置 · 存档 → 我的 AI 模型」里配置本机 Ollama、DeepSeek 或自定义兼容接口，支持获取模型列表和手动填写模型名称。对话和每日小书可使用玩家选择的模型，偏好与生活足迹为陪伴提供上下文。
...
这是本地单人原型。AI 目前用于对话与每日小书；旅行手记、旅册章节和途中信件使用本地文本，明信片插画是预生成资源，不能将它们视为每次旅行都实时生成。AI 文本不会直接发放物品或修改奖励。
